...
export async function DELETE(req: NextRequest) {
  const activeWorkspaceId =
    req.cookies.get("activeWorkspaceId")?.value;
  ...
  try {
    ...
    /*
     * ---------------------------------------------------------
     * FIND ANOTHER WORKSPACE OWNED BY THE USER
     * ---------------------------------------------------------
     *
     * The user must have another workspace where they are
     * the OWNER.
     *
     * If this is their only owned workspace, deletion is blocked.
     */
    ...
    if (!fallbackMembership) {
      throw new WorkspaceDeletionError("You cannot delete this workspace because you must have another workspace that you own", 409);
    }

    const nextWorkspaceId = fallbackMembership.workspace;
     ...
     * ---------------------------------------------------------
     *
     * Only update the current user's default workspace if
     * the workspace being deleted is currently their default.
     */
    ...
    await User.updateOne(
      ...
      {
        $set: {
          defaultWorkspace: nextWorkspaceId,
        },
      }
    );
    ...
    mediaDeletionResults.forEach(
      (result, index) => {
        if (result.status === "rejected") {
          console.error(`Failed to delete Cloudinary asset ${cloudinaryPublicIds[index]}:`, result.reason);
        }
      }
    );
...
//           throw new WorkspaceDeletionError("This workspace cannot be deleted because every member must keep at least one workspace", 409);
//         };
