...
// LOADING SKELETON & OTHER LOADING UI
...
export default function EditBlogPage() {
  ...
  const { 
    ...
    //   Fetching the blog with id 
    useEffect (() => {

      fetchCategories();  // Fetch categories on load
      if (id) getBlogById(String(id));
      
    }, [id]);


    // SETTING THE OF THE FETCHED BLOG BY ID
    useEffect(() => {
    if (!singleBlog) return;  // If no blog fetched by id return

        // Ignore stale blog left in context
        if (singleBlog._id !== String(id)) return;

        setTitle(singleBlog.title || "");
        setSlug(singleBlog.slug || "");

        // Content is now expected to be an array
        setContent(
        ...
        // Hero image is now { file, preview }
        setHeroImage({
            file: null,
            preview: singleBlog.heroImage || "",
        });
        ...
        setUpdatedAt(
            ...
            ? new Date(singleBlog.updatedAt)
            : null
        );

        // Category and tags
        setSelectedCategory(singleBlog.category?.name || "");

        setSelectedTags(
          singleBlog.tags?.map((tag) => tag.name) || []
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
        ...
        // CASE 2: Image URL (existing image or Unsplash)
        ...
        try {
            ...
            // Only redirects when update succeeds
            router.push("/blogs");
        } catch (error) {
            // Error toast is already handled inside updateBlog
