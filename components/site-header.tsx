'use client';

import { usePathname } from "next/navigation";

import { Separator } from "@/components/ui/separator"
import { SidebarTrigger } from "@/components/ui/sidebar"

import { ThemeToggle } from "@/app/blocks/theme-toggle";


export function SiteHeader() {

  const pathname = usePathname();

  let headerName = "Blogs";

  if (pathname === "/dashboard") {
    headerName = "All Blogs";
  }

  if (pathname === "/dashboard/create") {
    headerName = "Create Blog";
  }

  if (pathname.startsWith("/dashboard/edit")) {
    headerName = "Edit Blog";
  }
  
  return (
    // <header className="flex h-(--header-height) shrink-0 items-center gap-2 border-b transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-(--header-height)">
    <header className="flex h-(--header-height) shrink-0 items-center justify-center gap-2 border rounded-md pl-4 my-2 mx-4 bg-card transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-(--header-height)">
      <div className="flex w-full items-center gap-1 px-4 lg:gap-2 lg:px-6">
        <SidebarTrigger className="-ml-1" />
        <Separator
          orientation="vertical"
          className="mx-2 data-[orientation=vertical]:h-4 border"
        />

        <div className="flex justify-between w-full items-center">
          <h1 className="text-base font-medium">{headerName}</h1>
          <ThemeToggle />
        </div>

      </div>
    </header>
  )
}
